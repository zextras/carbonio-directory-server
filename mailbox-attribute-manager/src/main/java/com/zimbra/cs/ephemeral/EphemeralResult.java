...
public class EphemeralResult {
    private static final EphemeralLogger NO_OP_LOGGER = (format, args) -> {};

    private EphemeralKey key;
    private String[] values;
    private final EphemeralLogger logger;

    public EphemeralResult(EphemeralKey key, String value, EphemeralLogger logger) {
        this.key = key;
        this.values = value == null ? null : new String[] {value};
        this.logger = logger;
    }

    public EphemeralResult(EphemeralKey key, String[] values, EphemeralLogger logger) {
        this.key = key;
        this.values = values;
        this.logger = logger;
    }

    public EphemeralResult(EphemeralKeyValuePair entry, EphemeralLogger logger) {
        this.key = entry.getKey();
        this.values = entry == null ? null : new String[] { entry.getValue() };
        this.logger = logger;
    }

    public EphemeralResult(EphemeralKeyValuePair[] entries, EphemeralLogger logger) {
        this.logger = logger;
    ...
    public EphemeralResult(EphemeralKey key, List<String> values, EphemeralLogger logger) {
        this(key, values.toArray(new String[0]), logger);
    }
    ...
    public Integer getIntValue() {
        try {
            ...
            logger.warn("value '%s' cannot be converted to an integer, returning null", values[0]);
            return null;
        }
    }
    ...
    public Integer getIntValue(Integer defaultValue) {
        try {
            ...
            logger.warn("value '%s' cannot be converted to an integer, returning default value", values[0]);
            return defaultValue;
        }
    }
    ...
    public Long getLongValue() {
        try {
            ...
            logger.warn("value '%s' cannot be converted to a long, returning null", values[0]);
            return null;
        }
    }
    ...
    public Long getLongValue(Long defaultValue) {
        try {
            ...
            logger.warn("value '%s' cannot be converted to a long, returning default value", values[0]);
            return defaultValue;
        }
    }
    ...
    public Boolean getBoolValue() {
        try {
            ...
            logger.warn("value '%s' cannot be converted to a boolean, returning null", values[0]);
            return null;
        }
    }
    ...
    public Boolean getBoolValue(Boolean defaultValue) {
        try {
            ...
            logger.warn("value '%s' cannot be converted to a boolean, returning default value", values[0]);
            return defaultValue;
        }
    }
    ...
    public static EphemeralResult emptyResult(EphemeralKey key) {
        return new EphemeralResult(key, new String[0], NO_OP_LOGGER);
    }
    ...
    private Integer[] convertToInts(Integer defaultValue) {
        ...
        for (int i = 0; i < values.length; i++ ) {
            try {
                ...
                logger.warn("value '%s' cannot be converted to an integer, using default", values[i]);
    ...
    private Long[] convertToLongs(Long defaultValue) {
        ...
        for (int i = 0; i < values.length; i++ ) {
            try {
                ...
                logger.warn("value '%s' cannot be converted to a long, using default", values[i]);
    ...
    private Boolean[] convertToBools(Boolean defaultValue) {
        ...
        for (int i = 0; i < values.length; i++ ) {
            try {
                ...
                logger.warn("value '%s' cannot be converted to a boolean, using default", values[i]);
