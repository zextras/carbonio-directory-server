...
public class AttributeManagerUtil {
  ...
  private static void generateEnum(StringBuilder result, AttributeInfo ai) {
    ...
    result.append(
        String.format(
            "        public static %s fromString(String s) throws IServiceException {%n", enumName));
    ...
    result.append(
        String.format(
            "             throw IServiceException.INVALID_REQUEST(\"invalid value: \"+s+\", valid"
          ...
          javaBody =
              String.format(
                  "try { String v = getEphemeralAttr(ZAttrProvisioning.A_%s, %s).getValue(); return"
                      ...
                      + " catch(com.zextras.entry.IServiceException e) { return %s; }",
      ...
      case TYPE_GENTIME:
        ...
        if (Boolean.TRUE.equals(ai.isEphemeral())) {
          javaBody =
              String.format(
                  "String v = getEphemeralAttr(ZAttrProvisioning.A_%s, %s).getValue(%s); return v"
                      + " == null ? null : parseGeneralizedTime(v);",
                  name, dynamic, defaultValue);
        } else {
          javaBody =
              String.format(
  ...
  public static void generateSetter(
    ...
    switch (type) {
      ...
      case TYPE_CERTIFICATE:
        javaType = "byte[]";
        putParam = String.format("%s==null ? \"\" : encodeLDAPBase64(%s)", name, name);
        break;
      case TYPE_INTEGER:
      case TYPE_PORT:
        javaType = "int";
    ...
    switch (setterType) {
      ...
      case ADD:
        if (Boolean.TRUE.equals(attributeInfo.isEphemeral())) {
          ...
          body =
              String.format(
                  "        addToMultiMap(attrs, \"+\"  + ZAttrProvisioning.A_%s, %s);%n",
                  name, name);
        }
